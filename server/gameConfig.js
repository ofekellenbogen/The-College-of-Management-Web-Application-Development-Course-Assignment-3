...
const gameStages = [
  {
    ...
    description: "הצג את כל המוצרים הקיימים במערכת.",
    instruction: "הצג את כל המוצרים הקיימים במערכת.",
    hint: "לשליפת משאבים משתמשים במתודת GET ובנתיב המשאב ברבים: /api/products",
    expectedMethod: "GET",
  ...
  },
  {
    ...
    description: "הצג את פרטי המוצר בעל מזהה (ID) מספר 1.",
    instruction: "הצג את פרטי המוצר בעל מזהה (ID) מספר 1.",
    hint: "ב-REST, מזהה משאב ספציפי מועבר כ-Route Parameter בנתיב הבקשה: מתודת GET לנתיב /api/products/1",
    expectedMethod: "GET",
  ...
  },
  {
    ...
    description: "נסה לצפות במוצר שאינו קיים במערכת (מזהה 999) ובחן את תגובת השגיאה המתקבלת מהשרת.",
    instruction: "נסה לצפות במוצר שאינו קיים במערכת (מזהה 999) ובחן את תגובת השגיאה המתקבלת מהשרת.",
    hint: "שלח בקשת GET לנתיב של מוצר לא קיים (/api/products/999). השרת יחזיר קוד סטטוס 404 (Not Found) במבנה JSON.",
    expectedMethod: "GET",
  ...
  },
  {
    ...
    description: "הצג רק את המוצרים השייכים לקטגוריית 'Electronics'.",
    instruction: "הצג רק את המוצרים השייכים לקטגוריית 'Electronics'.",
    hint: "סינון משאבים מתבצע ע\"י שליחת Query Parameter בבקשת GET: הוסף ב-Query את המפתח category עם הערך Electronics (נתיב: /api/products?category=Electronics).",
    expectedMethod: "GET",
    expectedPath: "/api/products",
    requiresBody: false,
    expectedStatus: 200,
  ...
  },
  {
    ...
    description: "הצג מוצרים מקטגוריית 'Books' ממוינים לפי מחיר מהזול ביותר ליקר ביותר.",
    instruction: "הצג מוצרים מקטגוריית 'Books' ממוינים לפי מחיר מהזול ביותר ליקר ביותר.",
    hint: "שלח בקשת GET עם שילוב של Query Parameters: סינון לפי category=Books, מיון לפי sortBy=price, וכיוון מיון עולה order=asc (משורשרים עם &).",
    expectedMethod: "GET",
    expectedPath: "/api/products",
    requiresBody: false,
    expectedStatus: 200,
  ...
  },
  {
    ...
    description: "צור והוסף מוצר חדש לקטלוג: ספר בשם 'Node.js in Action', מקטגוריית 'Books', במחיר 34.00, שנמצא במלאי (inStock: true).",
    instruction: "צור והוסף מוצר חדש לקטלוג: ספר בשם 'Node.js in Action', מקטגוריית 'Books', במחיר 34.00, שנמצא במלאי (inStock: true).",
    hint: "יצירת משאב חדש מתבצעת במתודת POST לנתיב /api/products, כאשר נתוני המוצר נשלחים ב-Request Body בפורמט JSON עם המפתחות name, category, price ו-inStock.",
    expectedMethod: "POST",
    expectedPath: "/api/products",
    requiresBody: true,
    expectedStatus: 201,
  ...
  },
  {
    ...
    description: "עדכן את מחיר המקלדת (מוצר מזהה 4) ל-69.99 ש\"ח.",
    instruction: "עדכן את מחיר המקלדת (מוצר מזהה 4) ל-69.99 ש\"ח.",
    hint: "עדכון משאב קיים מתבצע במתודת PATCH (או PUT) לנתיב המוצר הספציפי /api/products/4, יחד עם Request Body ב-JSON הכולל את השדה: {\"price\": 69.99}.",
  ...
  },
  {
    ...
    description: "מחק לצמיתות את מוצר מזהה 7 (הספל) ממאגר הנתונים.",
    instruction: "מחק לצמיתות את מוצר מזהה 7 (הספל) ממאגר הנתונים.",
    hint: "מחיקת משאב מתבצעת באמצעות מתודת DELETE לנתיב המשאב הספציפי שברצונך להסיר: /api/products/7.",
  ...
  },
  {
    ...
    description: "הצג את כל חוות הדעת והביקורות (reviews) שנכתבו עבור מוצר מזהה 1.",
    instruction: "הצג את כל חוות הדעת והביקורות (reviews) שנכתבו עבור מוצר מזהה 1.",
    hint: "ייצוג קשר בין משאבים (Nested Resource) נעשה בנתיב היררכי: שלח בקשת GET לנתיב /api/products/1/reviews.",
    expectedMethod: "GET",
  ...
  },
  {
    ...
    description: "הוסף ביקורת חדשה למוצר מזהה 2 מאת 'Dana Ron' עם דירוג 5 וההערה 'Must have for developers!'.",
    instruction: "הוסף ביקורת חדשה למוצר מזהה 2 מאת 'Dana Ron' עם דירוג 5 וההערה 'Must have for developers!'.",
    hint: "יצירת משאב מקושר מתבצעת במתודת POST לנתיב ההיררכי /api/products/2/reviews עם גוף הבקשה (JSON Body) המכיל author, rating, comment.",
    expectedMethod: "POST",
